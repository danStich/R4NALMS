...
## **Working with code**
We will spend a bit of time during the introductory session to orient attendees with the RStudio integrated development environment (IDE) and talk about file management. Here are some additional pointers about working with code.
...
# done or what you are doing and why.
...
4. Once you've run a code block you can change it and then press the button next to `Run` (or `Ctrl + Alt + P`)  to 're-run' the previous block of code.
...
Some names are reserved or pre-defined. Did you notice that R already knew what `T` and `TRUE` were without us defining them? These reserved objects, and built-in **functions** are the building blocks of the R language.
...
The `Home` button will move you to the start of the line of code, and to the start of the line if you press it twice. The `End` button on your keyboard will take you to the end of a line. These will help you move around your code faster. If you don't have them you're cooked, but there is probably a speed key combination you can use.
...
Perhaps more practically speaking, we can do element-wise math operations on vectors easily in R.
```{r}
a * .5 # Multiplication
a + 100 # Addition
a - 3 # Subtraction
...
### **Dataframes**
Dataframes are like matrices, only not. They have a row/column structure like matrices. But, they can hold more than one data type! They are basically a whole bunch of vectors that are smashed together into a single container (a list, a special kind of vector). 
...
You will probably get a bunch of messages to tell you which packages have been loaded and if there were any `NAMESPACE` conflicts of functions that are "masked" from base R. None of these will affect us. I just told R not to print it on the website because it takes up space.
